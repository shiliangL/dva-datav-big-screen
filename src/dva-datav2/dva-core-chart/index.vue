 ...
 * @LastEditTime: 2022-08-06 07:55:36
...
export default {
  ...
  activated () {
    if (this.autoResize) this.resizeDvaChart()
  },
  created () {
    // 合并装饰器配置
    Object.assign(this.initDecoratorConfig, this.decoratorConfig)
  ...
  methods: {
    initDvaChart () {
      ...
      const { option, initDecoratorConfig } = this
      const chart = this.$el.querySelector('.dva-core-chart-canvas')
      this._chart = echarts.init(chart, this.theme, this.initOptions)
      ...
      if (initDecoratorConfig.show) {
        ...
        this._decoratorChart = echarts.init(decoratorChart, null, this.initOptions)
