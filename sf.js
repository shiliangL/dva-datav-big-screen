const name_title = 'Top Province of Total Population in China'
const subname = '虚拟数据(随便写的)'
const nameColor = '#4444FF'
const name_fontFamily = '等线'
const name_fontSize = 25
...
// 柱状图数据处理
const yData = []
let barData = data.filter(function (item) {
  return item.value > 0
})
      ...
      label: {
        normal: {
          formatter: function (obj) {
            const name = transferProvinceName(obj.name)
            return name || ''
          },
          position: 'right',
          show: true
...
function transferProvinceName (name) {
  const provincesPinyin = ['Shanghai', 'Hebei', 'Shanxi', 'Neimenggu', 'Liaoning', 'Jilin', 'Heilongjiang', 'Jiangsu', 'Zhejiang', 'Anhui', 'Fujian', 'Jiangxi', 'Shandong', 'Henan', 'Hubei', 'Hunan', 'Guangdong', 'Guangxi', 'Hainan', 'Sichuan', 'Guizhou', 'Yunnan', 'Xizang', 'Shanxi1', 'Gansu', 'Qinghai', 'Ningxia', 'Xinjiang', 'Beijing', 'Tianjin', 'Chongqing', 'Xianggang', 'Aomen', 'Taiwan']
  const provincesChinese = ['上海', '河北', '山西', '内蒙古', '辽宁', '吉林', '黑龙江', '江苏', '浙江', '安徽', '福建', '江西', '山东', '河南', '湖北', '湖南', '广东', '广西', '海南', '四川', '贵州', '云南', '西藏', '陕西', '甘肃', '青海', '宁夏', '新疆', '北京', '天津', '重庆', '香港', '澳门', '台湾']
  const pinyinName = provincesPinyin[provincesChinese.indexOf(name)]
  return pinyinName || ''
}
